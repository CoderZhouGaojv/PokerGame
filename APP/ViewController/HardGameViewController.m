...
@interface HardGameViewController ()<takeSelect,UIAlertViewDelegate>
...
@implementation HardGameViewController
...
- (void)viewDidLoad
{
    ...
    self.navigationItem.title = @"困难模式";
...
//    self.activedUser.hardGameHighestScore = 52;
//    self.activedUser.openedVeryHard = YES;
}
...
-(void)finshGameOpenAllPokersWithTimes
{
    if (self.myGameLogic.score > self
        .activedUser.hardGameHighestScore)
    {
        self
        .activedUser.hardGameHighestScore = self.myGameLogic.score;
    }
    
    //如果本局分数高于50分，该用户开启隐藏模式（极难模式）
    if (self.myGameLogic.score > 50)
    {
        self.activedUser.openedVeryHard = YES;
    }
